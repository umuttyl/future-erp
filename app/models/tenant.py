...
class Tenant(Base):
    ...
    # Şirket profili bilgileri (2B-11)
    address: Mapped[Optional[str]] = mapped_column(Text, nullable=True, default=None)
    phone: Mapped[Optional[str]] = mapped_column(String(64), nullable=True, default=None)
    tax_number: Mapped[Optional[str]] = mapped_column(String(64), nullable=True, default=None)
    bank_info: Mapped[Optional[str]] = mapped_column(Text, nullable=True, default=None)
    logo_url: Mapped[Optional[str]] = mapped_column(String(512), nullable=True, default=None)

    # Yerelleştirme (global iş temeli) — para birimi ve dil tercihi
    # currency: ISO 4217 kodu (TRY, USD, EUR...). Tüm parasal gösterim/raporlama bunu temel alır.
    currency: Mapped[str] = mapped_column(String(3), nullable=False, default="TRY", server_default="TRY")
    # language: ISO 639-1 kodu (tr, en...). UI dili + AI yanıt dili tercihi.
