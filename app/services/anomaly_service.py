        ...
        if p_score < -0.15:
            ...
            results.append(
                AnomalyResult(
                    source="sales",
                    severity=severity,
                    title="Abnormal Unit Price",
                    message=(
                        f"Unexpected price for {row.product_name}: "
                        f"{row.unit_price:.2f} (outside normal range)"
                    ),
                    score=p_score,
                    entity_id=row.id,
        ...
        if q_score < -0.15:
            ...
            results.append(
                AnomalyResult(
                    source="sales",
                    severity=severity,
                    title="Abnormal Sales Quantity",
                    message=(
                        f"Unexpected sales quantity for {row.product_name}: "
...
def detect_inventory_anomalies(db: Session, *, tenant_id: int) -> list[AnomalyResult]:
    ...
    for row, score in zip(rows, scores):
        if score < -0.15:
            ...
            direction = "drop" if row.change < 0 else "increase"
            results.append(
                AnomalyResult(
                    source="inventory",
                    severity=severity,
                    title=f"Abnormal Stock {direction.capitalize()}",
                    message=(
                        f"Unexpected stock {direction} for {row.product_name}: "
                        f"{abs(row.change)} units ({row.movement_type})"
                    ),
                    score=score,
...
def anomaly_result_to_ws_payload(result: AnomalyResult) -> dict[str, Any]:
    ...
    if product_id:
        payload["action_label"] = "Create Order Draft"
