...
def _is_smalltalk_greeting(text: str) -> bool:
    ...
    return any(
        x in t
        for x in (
            "merhaba", "selam", "hey", "günaydın", "gunaydin",
            "iyi günler", "iyi aksamlar", "nasılsın", "nasilsin",
            "ne yapabilirsin", "hi", "hello", "good morning",
    ...
    try:
        if _is_smalltalk_greeting(payload.text):
            return _friendly_response(
                "Hello! Welcome to Future ERP AI. How can I help you today?"
            )

        # Aktif modülleri ve şirket adını tenant'tan al (role-aware SQL whitelist + prompt için)
        active_modules: list[str] | None = None
        tenant_name: str = ""
        is_admin = principal.is_platform_admin if principal else False
        user_role = "admin" if is_admin else ("manager" if (principal and principal.role_kind == "owner") else "manager")
        if principal:
        ...
        # Admin platform sorguları expert routing'den geçmez — cross-tenant prompt doğrudan kullanılır.
        if is_admin:
            ...
            expert_key = auto_route_expert(payload.text)
