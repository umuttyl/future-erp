...
def test_me_as_owner(client, test_admin):
    """client fixture artık tenant owner — /me 'manager' rolü ve tam izinler döner."""
    r = client.get("/api/auth/me")
    assert r.status_code == 200
    data = r.json()
    assert data["email"] == test_admin.email
    assert data["role"] == "manager"          # owner → 'manager' etiketi
    assert data["is_platform_admin"] is False
    assert data["role_kind"] == "owner"
    assert data.get("full_name") == "Test Admin"
    assert "admin.access" in data["permissions"]


def test_me_as_platform_admin(client_platform_admin, test_platform_admin):
    """Gerçek platform admin → role 'admin', is_platform_admin True, tenant_id null."""
    r = client_platform_admin.get("/api/auth/me")
    assert r.status_code == 200
    data = r.json()
    assert data["email"] == test_platform_admin.email
