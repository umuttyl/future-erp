...
def _build_token(payload_overrides: dict) -> str:
    """JWT_SECRET ile imzalanmış özel payload token'ı üretir (varsayılan: tenant owner)."""
...
def test_jwt_payload_uses_tid_not_tenant_id(test_tenant: Tenant, test_admin: User):
    ...
    assert "role" not in payload, "JWT 'role' claim içermemeli (ADM-10: is_platform_admin + role_kind kullanılır)"
...
def test_ws_accepts_valid_admin_token(db_session: Session, test_tenant: Tenant, test_admin: User):
    """Geçerli access token (tenant owner) → WS bağlantısı kurulmalı."""
    token = create_access_token(
        user_id=test_admin.id,
        tenant_id=test_tenant.id,
        is_platform_admin=False,
...
def test_ws_accepts_platform_admin_zero_tid(db_session: Session):
    """Platform admin (is_platform_admin=True, tid=0) → bağlantı kabul edilmeli."""
    token = _build_token({"tid": 0, "is_platform_admin": True, "role_kind": ""})
    client = _make_app(db_session)
    with client:
        with client.websocket_connect(f"/api/ws/notifications?access_token={token}"):
            pass  # Bağlantı başarılıysa test geçer
