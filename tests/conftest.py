...
def test_admin(db_session: Session, test_tenant: Tenant) -> User:
    """Tenant sahibi (owner) — tenant verilerine tam erişim.

    ADM-10 öncesi 'admin' rolüydü; artık tenant owner. Gerçek platform admin
    için ``test_platform_admin`` fixture'ını kullanın.
    """
    u = User(
...
@pytest.fixture
def test_platform_admin(db_session: Session) -> User:
    """Gerçek platform admin — tenant_id=NULL, is_platform_admin=True."""
    u = User(
        tenant_id=None,
...
def client(
    ...
    """get_db + tenant owner principal override (tenant verilerine tam erişim)."""
...
@pytest.fixture
def client_platform_admin(
    db_session: Session, test_platform_admin: User
) -> Generator[TestClient, None, None]:
    """Platform admin principal — tenant_id=0 sentinel, is_platform_admin=True."""
    application = create_app()
    principal = AuthPrincipal(
...
def client_for(db_session: Session):
    ...
    def _make(user: User) -> TestClient:
        ...
        principal = AuthPrincipal(
            user_id=user.id,
            tenant_id=user.tenant_id if user.tenant_id is not None else 0,
            email=user.email,
