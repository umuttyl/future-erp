...
export function SkeletonCard({ rows = 3 }: { rows?: number }) {
  return (
    <div className="rounded-2xl border border-ui-border-sub bg-ui-surface p-5 dark:border-white/[0.06]">
...
export function SkeletonTable({ rows = 6, cols = 4 }: { rows?: number; cols?: number }) {
  return (
    <div className="rounded-2xl border border-ui-border-sub bg-ui-surface dark:border-white/[0.06]">
      <div className="border-b border-ui-border-sub p-5">
      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {Array.from({ length: rows }).map((_, r) => (
            <tr key={r} className="border-b border-ui-border-sub">
  ...
  return (
    <div className={`grid gap-4 sm:grid-cols-2 lg:grid-cols-${count}`}>
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="rounded-2xl border border-ui-border-sub bg-ui-surface p-5 dark:border-white/[0.06]">
