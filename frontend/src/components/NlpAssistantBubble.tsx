...
const INTRO_MANAGER: ChatMessage = {
  ...
  kind: "text",
  text:
    "Hello! You can ask questions in natural language. Examples:\n" +
    '• "List the top 5 products by revenue"\n' +
    '• "Which products are below their stock threshold?"\n' +
    '• "Top 3 customers by sales in the last 30 days"',
};
...
const INTRO_ADMIN: ChatMessage = {
  ...
  kind: "text",
  text:
    "Hello! You can query platform-wide data. Examples:\n" +
    '• "How many active companies are there and in which industries?"\n' +
    '• "Top 5 products by revenue across all companies"\n' +
    '• "Total sales comparison by company"',
};
...
export function NlpAssistantBubble() {
  ...
  async function send() {
    ...
    try {
      ...
      const detail = getApiErrorMessage(e, "Request failed");
      setMessages((m) => [
        ...m,
        {
          id: `e-${Date.now()}`,
        ...
        >
          ...
          <div className="shrink-0 border-t border-slate-200/90 bg-white/95 p-3 dark:border-slate-700/80 dark:bg-slate-900/95">
            ...
            <p className="mt-2 text-center text-[10px] text-slate-500 dark:text-slate-400">
              Responses are generated by AI; displayed text and tables are for informational purposes.
            </p>
          </div>
        </div>
      )}
          ...
          <div className="overflow-hidden rounded-xl border border-slate-200/80 bg-slate-50/90 dark:border-slate-700/60 dark:bg-slate-900/60">
            ...
            {data.length > 25 && (
              <div className="bg-slate-100/80 px-2 py-1 text-[10px] text-slate-500 dark:bg-slate-900/70 dark:text-slate-400">
                Showing first 25 rows (total {data.length}).
              </div>
            )}
          </div>
        )}
