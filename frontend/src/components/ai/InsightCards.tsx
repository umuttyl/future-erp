...
export function InsightBanner({
  ...
  return (
    <div className="rounded-2xl border border-violet-300/40 bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 p-6 text-white shadow-md dark:border-violet-500/25">
      <div className="text-xs font-semibold uppercase tracking-wide text-violet-200">{t('ai.headline')}</div>
      ...
      {insights && (
        <div className="mt-3 flex flex-wrap items-center gap-3 text-xs">
          {insights.context.total_tenants != null ? (
            ...
            <>
              <Chip label={t('ai.chipPlatformRevenue')} value={formatCurrency(insights.context.summary_last_30.revenue)} />
              <Chip
                label={t('ai.chipActiveCompanies')}
                value={formatNumber(insights.context.total_tenants)}
              />
              <Chip
                label={t('ai.chipCriticalStock')}
              ...
              />
              <Chip label={t('ai.chipPlatformUsers')} value={formatNumber(insights.context.total_users ?? 0)} />
            </>
          ) : (
            // Regular: per-tenant chips
            <>
              <Chip label={t('ai.chipRevenue30')} value={formatCurrency(insights.context.summary_last_30.revenue)} />
              <Chip
                label={t('ai.chipRevenueChange')}
              ...
              />
              <Chip label={t('ai.chipActiveSku')} value={formatNumber(insights.context.total_skus ?? 0)} />
            </>
          )}
        </div>
      )}
  ...
  return (
    <GlobalCard className={['border-l-4', cfg.borderLeft].join(' ')}>
      <div className="flex items-center gap-2">
        ...
        <div className={['text-xs font-semibold uppercase tracking-wide', cfg.label].join(' ')}>{t(cfg.nameKey)}</div>
      </div>
...
function HighlightSkeleton() {
  return (
    <GlobalCard className="animate-pulse">
      <div className="h-3 w-16 rounded bg-ui-surface-2" />
      <div className="mt-3 h-4 w-3/4 rounded bg-ui-surface-2" />
      <div className="mt-2 h-3 w-full rounded bg-ui-surface-2" />
      <div className="mt-1 h-3 w-2/3 rounded bg-ui-surface-2" />
    </GlobalCard>
  )
}
...
function severityConfig(severity: AiHighlight['severity']) {
  switch (severity) {
    case 'positive':
      return {
        nameKey: 'ai.sevPositive', borderLeft: 'border-l-emerald-500', dot: 'bg-emerald-500',
        label: 'text-emerald-700 dark:text-emerald-400',
        titleClass: 'text-ui-text',
        bodyClass: 'text-ui-muted',
        metricBox: 'border-ui-border bg-ui-surface-2 text-ui-text',
      }
    case 'warning':
      return {
        nameKey: 'ai.sevWarning', borderLeft: 'border-l-amber-500', dot: 'bg-amber-500',
    ...
    case 'critical':
      return {
        nameKey: 'ai.sevCritical', borderLeft: 'border-l-rose-500', dot: 'bg-rose-500',
    ...
    default:
      return {
        nameKey: 'ai.sevInfo', borderLeft: 'border-l-sky-500', dot: 'bg-sky-500',
