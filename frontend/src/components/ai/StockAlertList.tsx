...
export function StockAlertList({ alerts }: { alerts: StockAlert[] }) {
  ...
  return (
    <GlobalCard>
      <GlobalCardHeader
        title="Stock Alerts"
        description={isCrossTenant ? 'All companies — below-threshold products' : 'Below-threshold products'}
      />
      {alerts.length === 0 ? (
        <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-900 dark:border-emerald-500/30 dark:bg-emerald-950/40 dark:text-emerald-100">
          No products at critical stock level right now.
        </div>
      ) : (
          ...
          {alerts.map((a) => (
            ...
            >
              ...
              <div className="text-right">
                ...
                <div className="text-[10px] text-slate-500">stock / threshold</div>
              </div>
            </li>
          ))}
        </ul>
