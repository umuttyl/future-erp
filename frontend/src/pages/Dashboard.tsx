...
function sumDailyRevenue(points: DailySalesPoint[]) {
  return points.reduce(
    (s, p) =>
      s + (typeof p.revenue === "number" ? p.revenue : Number(p.revenue)),
    0,
  );
}
...
function trendPct(
  cur: number,
  prev: number,
): { pct: number | null; positive: boolean } {
...
export function DashboardPage() {
            ...
            (async () => {
              const [a, b, c, d] = await Promise.all([
                api.get<DailySalesPoint[]>("/sales/analytics/daily", {
                  params: { start_date: startCur, end_date: endCur },
                }),
                api.get<DailySalesPoint[]>("/sales/analytics/daily", {
                  params: { start_date: startPrev, end_date: endPrev },
                }),
                api.get<unknown[]>("/sales/records", {
                  params: {
                    start_date: startCur,
                    end_date: endCur,
                    limit: 500,
  ...
  const criticalNow = products.filter(
  ...
  const aiScore = useMemo(() => {
    const pts = chartData
      .filter((x) => x.actualQty != null && x.forecastQty != null)
      .slice(-14);
    if (pts.length < 3)
      return { score: null as number | null, prev: null as number | null };
    let err = 0;
    let denom = 0;
    for (const x of pts) {
    ...
    const prevScore = d2
      ? Math.max(0, Math.min(100, Math.round((1 - err2 / d2) * 100)))
      : score;
    ...
    <PageLayout
      ...
      actions={
        <div className="flex flex-wrap items-center gap-2">
          <span
            className={secondaryButtonClass + " pointer-events-none text-xs"}
          >
