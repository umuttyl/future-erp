...
export function InvoicePrintPage() {
  ...
  useEffect(() => {
    ...
    fetchInvoicePrintData(parseInt(orderId), parseInt(invoiceId))
      .then(setData)
      .catch(() => setError('Fatura verisi yüklenemedi.'))
  }, [orderId, invoiceId])

  useEffect(() => {
    if (data) {
      document.title = `Fatura ${data.invoice_no}`
    }
  }, [data])
    ...
    <>
      ...
      <div className="no-print fixed inset-x-0 top-0 z-50 flex items-center justify-between border-b border-slate-200 bg-white px-6 py-3 shadow-sm print:hidden">
        <span className="text-sm font-medium text-slate-700">
          Fatura: <span className="font-bold text-slate-900">{data.invoice_no}</span>
        </span>
      ...
      <div className="invoice-page mx-auto min-h-screen bg-white px-12 pb-16 pt-20 print:px-8 print:pt-8" style={{ maxWidth: 860 }}>
        ...
        <div className="mb-8 flex items-start justify-between">
          <div>
            ...
            {data.company_tax_number && (
              <p className="text-sm text-slate-600">VKN: {data.company_tax_number}</p>
            )}
          </div>
          <div className="text-right">
            ...
            <p className="mt-1 text-sm text-slate-500">Tarih: {fmtDate(data.invoice_date)}</p>
            <p className="text-sm text-slate-500">Sipariş: {data.order_no} ({fmtDate(data.order_date)})</p>
          </div>
        </div>
        ...
        {/* Bill to */}
        <div className="mb-8">
          <p className="mb-1 text-xs font-semibold uppercase tracking-widest text-slate-400">Fatura Kesilen</p>
        ...
        <table className="mb-6 w-full border-collapse text-sm">
          <thead>
            <tr className="border-b-2 border-slate-200 bg-slate-50 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">
              <th className="py-2 pl-2 pr-4">Ürün / Açıklama</th>
              <th className="py-2 px-4 text-right">Miktar</th>
              <th className="py-2 px-4 text-right">Birim Fiyat</th>
              <th className="py-2 px-4 text-right">KDV %</th>
              <th className="py-2 px-4 text-right">KDV</th>
              <th className="py-2 pl-4 pr-2 text-right">Toplam</th>
            </tr>
          </thead>
          <tbody>
        ...
        <div className="mt-10 border-t border-slate-200 pt-4 text-center text-xs text-slate-400">
          Bu belge {data.company_name} tarafından düzenlenmiştir. • {data.company_tax_number ? `VKN: ${data.company_tax_number}` : ''}
        </div>
      </div>
