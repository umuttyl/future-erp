...
function SupplierModal({
  ...
  async function onSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!form.name.trim()) { setErr(t('suppliers.nameRequired')); return }
    setErr(null)
    setBusy(true)
    try {
      const payload = {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/55 p-4 backdrop-blur-sm" onClick={onClose}>
      <div className="relative w-full max-w-lg rounded-2xl border border-slate-200/90 bg-white shadow-2xl dark:border-white/10 dark:bg-[#1a1628]" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="border-b border-slate-100 px-6 py-5 dark:border-white/10">
          <h2 className="text-lg font-semibold text-slate-900 dark:text-white">{editing ? t('suppliers.editTitle') : t('suppliers.newTitle')}</h2>
        </div>
        <form className="grid gap-3 px-6 py-5" onSubmit={onSubmit}>
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2">
              <label htmlFor="sm-name" className="text-xs font-medium text-slate-600 dark:text-slate-400">{t('suppliers.companyNameField')}</label>
              <input id="sm-name" required className={inputFieldClass + ' mt-1'} value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
            </div>
            <div>
              <label htmlFor="sm-contact" className="text-xs font-medium text-slate-600 dark:text-slate-400">{t('suppliers.contactPerson')}</label>
            ...
            <div>
              <label htmlFor="sm-email" className="text-xs font-medium text-slate-600 dark:text-slate-400">{t('suppliers.email')}</label>
            ...
            <div>
              <label htmlFor="sm-phone" className="text-xs font-medium text-slate-600 dark:text-slate-400">{t('suppliers.phone')}</label>
            ...
            <div>
              <label htmlFor="sm-terms" className="text-xs font-medium text-slate-600 dark:text-slate-400">{t('suppliers.paymentTerms')}</label>
              <input id="sm-terms" className={inputFieldClass + ' mt-1'} value={form.payment_terms ?? ''} onChange={(e) => setForm({ ...form, payment_terms: e.target.value })} placeholder={t('suppliers.paymentTermsPlaceholder')} />
            </div>
            <div className="col-span-2">
              <label htmlFor="sm-notes" className="text-xs font-medium text-slate-600 dark:text-slate-400">{t('suppliers.notes')}</label>
          ...
          <div className="flex justify-end gap-2 border-t border-slate-100 pt-4 dark:border-white/10">
            <button type="button" onClick={onClose} className="rounded-lg border border-slate-200 px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 dark:border-white/10 dark:text-slate-300">{t('common.cancel')}</button>
...
export function SuppliersPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(getApiErrorMessage(e, t('suppliers.loadError')))
    } finally {
      setLoading(false)
    }
  }, [t])
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      setError(getApiErrorMessage(e, t('suppliers.deleteFailed')))
    } finally {
      setDeleting(false)
    }
  }
  ...
  function handleExport() {
    downloadCsv(
      filtered.map((s) => ({
        ID: s.id,
        [t('suppliers.companyName')]: s.name,
        [t('suppliers.contactPerson')]: s.contact_person ?? '',
        [t('suppliers.email')]: s.email ?? '',
        [t('suppliers.phone')]: s.phone ?? '',
        [t('suppliers.paymentTerms')]: s.payment_terms ?? '',
        [t('suppliers.notes')]: s.notes ?? '',
        [t('customers.colCreatedAt')]: formatDate(s.created_at),
      })),
      'tedarikciler.csv',
    )
  }
        ...
        <GlobalCard>
          ...
          {filtered.length === 0 ? (
            <EmptyState
              ...
              description={search ? t('suppliers.emptySearchHint') : t('suppliers.emptyHint')}
              action={
                canWrite ? (
                ...
                <table className="w-full text-left text-sm">
                  <thead>
                    <tr className={`border-b border-slate-200 dark:border-white/10 ${tableHeaderClass}`}>
                      ...
                      {canWrite && <th className="py-2 pr-5 sr-only">{t('common.actions')}</th>}
                    </tr>
                  </thead>
                  <tbody>
                    {paged.map((s) => (
                        ...
                        {canWrite && (
                          <td className="py-2.5 pr-5">
                            <div className="flex items-center gap-1">
                              <button type="button" aria-label={t('common.edit')} onClick={() => { setEditTarget(s); setModalOpen(true) }} className="flex h-8 w-8 items-center justify-center rounded-lg text-slate-500 hover:bg-violet-50 hover:text-violet-600 dark:text-slate-400 dark:hover:bg-violet-500/10 dark:hover:text-violet-300">
                                <Pencil className="h-3.5 w-3.5" />
                              </button>
                              <button type="button" aria-label={t('common.delete')} onClick={() => setDeleteTarget(s)} className="flex h-8 w-8 items-center justify-center rounded-lg text-slate-500 hover:bg-rose-50 hover:text-rose-600 dark:text-slate-400 dark:hover:bg-rose-500/10 dark:hover:text-rose-400">
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/55 p-4 backdrop-blur-sm" onClick={() => setDeleteTarget(null)}>
          <div className="w-full max-w-sm rounded-2xl border border-slate-200/90 bg-white p-6 shadow-2xl dark:border-white/10 dark:bg-[#1a1628]" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-base font-semibold text-slate-900 dark:text-white">{t('suppliers.deleteConfirm')}</h2>
            <p className="mt-1 text-sm text-slate-600 dark:text-slate-400"><strong>{deleteTarget.name}</strong></p>
            <div className="mt-4 flex justify-end gap-2">
              <button type="button" onClick={() => setDeleteTarget(null)} className="rounded-lg border border-slate-200 px-4 py-2 text-sm text-slate-700 dark:border-white/10 dark:text-slate-300">{t('common.cancel')}</button>
